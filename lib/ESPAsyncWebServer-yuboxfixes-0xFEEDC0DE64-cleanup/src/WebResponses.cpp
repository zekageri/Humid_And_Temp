...
AsyncFileResponse::AsyncFileResponse(FS &fs, const String& path, const String& contentType, bool download, AwsTemplateProcessor callback): AsyncAbstractResponse(callback){
  ...
  char* filename = (char*)path.c_str() + filenameStart;
    ...
    //Serial.printf("[ASYNC INSIDE] - FILE NAMEEEEE %s\n", filename);

  // Convert the filename to a C++ string
  std::string filenameStr(filename);

  // Find the position of the ".gz" extension in the string
  size_t dotGzPos = filenameStr.find(".gz");

  // Check if the ".gz" extension was found
  if (dotGzPos != std::string::npos) {
      // Remove the ".gz" extension from the string
      filenameStr.erase(dotGzPos, 3); // 3 is the length of ".gz"
      
      // Convert the modified string back to a char* if needed
      strcpy(filename, filenameStr.c_str());
  }

  //Serial.printf("(1) [ASYNC INSIDE] - FILE NAMEEEEE %s\n", filename);
